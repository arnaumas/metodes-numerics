...
int main(int argc, char* argv[]) {
  ...
  if((c = fgetc(fitxer)) != '\n') {
	...
	//Assignem la memòria
	dades = (float*) malloc(n*sizeof(double));		

	//Omplim 'dades' amb els valors del fitxer
	int i;
	for(i = 0; i < n; i++) {
		if(fscanf(fitxer, "%f", &dades[i]) != 1) {
	...
	float mitja = mitja_fl(dades, n);
	float var = var2_fl(dades, mitja, n);

	printf("La variança del conjunt de dades és: %.9g\n", var);
}
